  ...
  const analizarFotos = async () => {
    ...
    try {
      ...
      if (mostrador) {
        ...
        await Promise.all(
          photos.map(async (photo: ImagePicker.ImagePickerAsset) => {
            const mimeType =
              photo.uri.match(/data:(.*?);base64/)?.[1] || "image/jpeg"; // Asumimos que es JPEG si no se puede determinar
          ...
          <Grid2 container columns={3} spacing={2}>
            {photos.length <= 0 && (
              <Text style={[styles.buttonText, { color: "red" }]}>
                {"Por favor cargue fotografías"}
              </Text>
            )}

            {photos &&
              photos.map((photo: any, indx: any) => (
            ...
            <Grid2 size={1}>
              <TouchableOpacity style={styles.button2} onPress={agregarFotos}>
                <Text style={[styles.buttonText, { color: "green" }]}>
                  {photos.length > 0 ? "Abrir cámara" : "Cargar desde cámara"}
                </Text>
              </TouchableOpacity>
            </Grid2>
            <Grid2 size={1}>
              <TouchableOpacity
                ...
                onPress={agregarFotoslibrary}
              >
                <Text style={[styles.buttonText, { color: "green" }]}>
                  {photos.length > 0 ? "Abrir galería" : "Cargar desde galería"}
                </Text>
              </TouchableOpacity>
            </Grid2>
            <Grid2 size={2}>
