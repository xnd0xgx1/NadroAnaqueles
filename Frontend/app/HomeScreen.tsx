...
const HistoryItem = ({
  ...
  const handleDelete = async () => {
    const confirmDelete = window.confirm(
      "¿Estás seguro de que deseas borrar esto?"
    );
      ...
      <Grid2 container columns={7} rowSpacing={2}>
        <Grid2 size={1}>
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center" }}>
              <Text style={[styles.statPercentage, { fontWeight: "700" }]}>
                {day}{" "}
              </Text>
            ...
            <Text style={[styles.statPercentage, { fontSize: 12 }]}>
              {hour}
            </Text>
          </View>
        </Grid2>
        ...
        {Status == "Listo" && (
          <>
            ...
            <Grid2 size={1}>
              <View
                style={{ height: "100%", backgroundColor: "#ccc", width: 1 }}
              />
            </Grid2>
            <Grid2 size={5}>
              <View
...
export default function HomeScreen() {
  ...
  const getData = async () => {
    const urlBase = `https://func-anaqueles-inteligentes-back.azurewebsites.net/api/Historial?mes=${mes}&anio=${anio}&user=${await GetValueFor(
      "user"
    )}`;
    let url = urlBase;
  ...
  const getstatdistics = async () => {
    ...
    const urlBase = `https://func-anaqueles-inteligentes-back.azurewebsites.net/api/Estadisticas?mes=${mes}&anio=${anio}&user=${await GetValueFor(
      "user"
    )}`;
    let url = urlBase;
  ...
  return (
    <>
        ...
        <>
            ...
            <Box display="flex" alignItems="center" gap={3}>
              <FormControl variant="standard">
                ...
                <Select
                  ...
                  sx={{ width: "20vw" }}
                  value={mostrador}
                  onChange={(value) => setMostrador(value.target.value)}
                >
                  <MenuItem value="Todos">Todos</MenuItem>
                  {mostradores.map((item) => (
          ...
          <Grid2 container columns={10} spacing={2}>
            <Grid2 size={{ sm: 6, xs: 10 }}>
              <View style={styles.statBox}>
                <Text style={styles.statTitle}>
                  {"Mostradores analizados " + nombresMeses[Number(mes) - 1]}
                </Text>
                <Text style={[styles.statPercentage]}>
                  {estadisticas.Numero_de_registros} / {"80"}
                </Text>
              </View>
            </Grid2>
            <Grid2 size={{ sm: 4, xs: 10 }}>
              <View style={styles.statBox}>
                <Text style={styles.statTitle}>
                  {"% Completitud anaqueles"}
                </Text>
                <Text style={[styles.statPercentage]}>
                  {estadisticas.Promedio_de_completitud
                    ? estadisticas.Promedio_de_completitud.toFixed(2)
                    : ""}
                  %
                </Text>
              </View>
            ...
            <Grid2 size={{ sm: 7, xs: 10 }}>
              <View style={styles.statBox}>
                <Text style={styles.statTitle}>
                  {"Productos anaqueles vs productos invenadro"}
                </Text>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  <Text style={[styles.statPercentage, { color: "red" }]}>
                    {estadisticas.Promedio_de_PInvenadro
                      ? estadisticas.Promedio_de_PInvenadro.toFixed(2)
                      : ""}
                    %
                  </Text>
                  ...
                  <Text style={[styles.statPercentage, { color: "green" }]}>
                    {(
                      ((estadisticas.Promedio_de_PInvenadro ?? 0) * 100) /
                      estadisticas.Promedio_de_PInvenadro
                    ).toFixed(2)}
                    %
                  </Text>
            ...
            <Grid2 size={{ sm: 3, xs: 10 }}>
              <View style={styles.statBox}>
                ...
                <Text style={[styles.statPercentage]}>
                  {estadisticas.Promedio_de_Ventas_Invenadro
                    ? estadisticas.Promedio_de_Ventas_Invenadro.toFixed(2)
                    : ""}{" "}
                  mxn
                </Text>
              </View>
      ...
      <TouchableOpacity
        onPress={() => router.push("/NuevoAnalisis")}
        style={styles.button}
      >
