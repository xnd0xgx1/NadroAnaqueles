...
import {
  ...
  MenuItem,
  Select,
} from "@mui/material"; // Asegúrate de que este import sea correcto
        ...
        {!seemore && (
          ...
          >
            ...
            <Text style={[styles.text, { color: "black" }]}>
              {nombre_oficial == "NaN" ? "No especificado" : nombre_oficial}
            </Text>
            <Text style={[styles.statPercentage, { color: "black" }]}>
              Porcentaje de coincidencia
            </Text>
            <Text style={[styles.text, { color: "black" }]}>
              {similitud == "NaN" ? "No especificado" : similitud.toFixed(2)} %
            </Text>
            <Text style={[styles.statPercentage, { color: "black" }]}>
              Existencias
            </Text>
...
const Recomendacionitem = ({
  Valor_Optimo_x,
  Optimo_x,
  Precio_Publico_Material,
  Precio_Farmacia_x,
  Precio_Publico_Invenadro,
  Fecha_Generacion,
  Descripcion,
  percentage,
  Estatus_Material,
  ...
  Descripcion: any;
  Precio_Publico_Material: any;
  Precio_Publico_Invenadro: any;
  percentage: any;
  Fecha_Generacion: any;
  Precio_Farmacia_x: any;
  optimo: any;
  Estatus_Material: any;
  Optimo_x: any;
  Valor_Optimo_x: any;
}) => {
  let image = require("@/assets/images/Home/Anaquelgreen.png");
...
export default function Resultados() {
  ...
  const getdata = async () => {
    ...
    const newimages = resultadosparsed.output_imagenes.map((image: any) => {
      return {
        path: image.ruta_img_original,
        porcentaje: image.porcentaje_calidad_imagen,
      };
    });
    const newimages2 = resultadosparsed.output_imagenes.map((image: any) => {
      return { path: image.ruta_img_anaquel, porcentaje: "" };
    });
  ...
  const saveresults = async () => {
    ...
    const resultadoitem = {
      ...
      PInvenadro: (
        ((resultado.productos_detectados_en_invenadro ?? 0) * 100) /
        resultado.total_productos_inferidos
      ).toFixed(0),
      Direccion: "",
    ...
    try {
      ...
      const url =
        ...
        "api/InsertarRegistro"; // Cambia esto según tu función
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center',width:"100%"}}>
            ...
            <Grid2 size={4} height={"100%"} alignContent={"left"}>
              <TouchableOpacity
                ...
                onPress={() => router.push("/SubirFotos")}
              >
                <Text
                  style={{
                    color: "green",
